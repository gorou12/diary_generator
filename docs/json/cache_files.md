...
- トピック本文中に現れる内容なしのブロックは、空段落として扱う。
  - 行間を開けたい時など見せ方のために使っているため
  - 変更通知の対象とする。
- 内容のあるトピックの後に現れる内容なしのブロックは、保存しない。
  - Notionでの編集時に前の見出しとの間隔を保つためであり、静的サイトでは不要なもののため
  - 同様の理由により、変更通知の対象にしない。
